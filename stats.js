...
// + variance/stddev arrays for goals, corners, cards
async function batchCalc(fixtures, tId) {
  ...
  let totalCrd=0;
  let totalShotsOn=0, totalShotsOff=0, totalOppShotsOn=0;
  let n=0, nCor=0, nCrd=0, nShots=0;

  // Per-game arrays for variance
  ...
  for(let i=0; i<recent.length; i++) {
    ...
    goalsArr.push(myGoals);
    goalsAgainstArr.push(oppGoals);
    ...
    if(st && st.length) {
      ...
      if(myCor !== null)  { totalCor += myCor; cornersArr.push(myCor); nCor++; }
      ...
      const totalCardsGame = myY + myR;
      cardsArr.push(totalCardsGame);
      totalCrd += totalCardsGame; nCrd++;
  ...
  return {
    ...
    // Variance data
    goalsArr, goalsAgainstArr, cornersArr, cardsArr,
    varGoals:        variance(goalsArr),
    sdGoals:         stdDev(goalsArr),
    varGoalsAgainst: variance(goalsAgainstArr),
    sdGoalsAgainst:  stdDev(goalsAgainstArr),
    varCorners:      variance(cornersArr),
    sdCorners:       stdDev(cornersArr),
    varCards:        variance(cardsArr),
    sdCards:         stdDev(cardsArr),
  };
}
...
async function buildIntel(tId,lg,s,isHome) {
  try {
    ...
    const gen   = allFix.slice(0, 8);     // last 8 for general stats (batchCalc uses up to 8)
    const split = allFix.filter(f=>(isHome?f.teams.home.id:f.teams.away.id)===tId).slice(0,6);
    const recent6 = allFix.slice(0,6);   // strictly last 6 for variance window label

    const [fData, sData, r6Data] = await Promise.all([
      batchCalc(gen,    tId),
      batchCalc(split,  tId),
      batchCalc(recent6, tId)   // last-6 variance window
    ]);

    const seasonXG  = parseFloat(ss?.goals?.for?.average?.total)     || 1.35;
    const seasonXGA = parseFloat(ss?.goals?.against?.average?.total) || 1.35;

    // ── Season variance from teams/statistics played & goals
    // API gives: goals.for.total.total, goals.against.total.total, fixtures.played.total
    ...
    // API provides minute-breakdown but not game-by-game; approximate σ from Poisson
    // For Poisson distribution: Var = λ, so σ ≈ √λ (theoretical lower bound)
    // Blend with empirical if we have last-8 data
    const seaLambdaGF  = seaPlayed > 0 ? seaGF  / seaPlayed : seasonXG;
    const seaLambdaGA  = seaPlayed > 0 ? seaGA  / seaPlayed : seasonXGA;
    // Season σ: Poisson theoretical if no per-game breakdown available
    const seaSdGoals        = Math.sqrt(seaLambdaGF);
    const seaSdGoalsAgainst = Math.sqrt(seaLambdaGA);
    // Season corners/cards: API doesn't break down by game; use Poisson approx on league avg
    const seaSdCorners = Math.sqrt(LEAGUE_CORNER_MEAN_H);
    const seaSdCards   = Math.sqrt(2.2); // rough league avg cards σ
    ...
    return {
      ...
      // ── Last-6 variance (empirical, from fixture statistics)
      r6: {
        n:              r6Data.goalsArr.length,
        sdGoals:        r6Data.sdGoals,
        sdGoalsAgainst: r6Data.sdGoalsAgainst,
        sdCorners:      r6Data.sdCorners,
        sdCards:        r6Data.sdCards,
        varGoals:       r6Data.varGoals,
        varCorners:     r6Data.varCorners,
        varCards:       r6Data.varCards,
        goalsArr:       r6Data.goalsArr,
        cornersArr:     r6Data.cornersArr,
        cardsArr:       r6Data.cardsArr,
      },

      // ── Season variance (Poisson-theoretical + API totals)
      sea: {
        n:              seaPlayed,
        avgGoals:       parseFloat(seaLambdaGF.toFixed(2)),
        avgGoalsAgainst:parseFloat(seaLambdaGA.toFixed(2)),
        sdGoals:        parseFloat(seaSdGoals.toFixed(2)),
        sdGoalsAgainst: parseFloat(seaSdGoalsAgainst.toFixed(2)),
        sdCorners:      parseFloat(seaSdCorners.toFixed(2)),
        sdCards:        parseFloat(seaSdCards.toFixed(2)),
      }
    };
  } catch {
    return {
      fXG:1.35, fXGA:1.35, sXG:1.35, formRating:50, corRatio:0.40,
      cor:4.5, crd:2.0, shotsOn:4.5, shotsOff:3.5, oppShotsOn:4.0,
      uiXG:'1.35', uiXGA:'1.35', uiSXG:'1.35', uiSXGA:'1.35', history:[],
...
// σ relative to league baseline → stability label
function volatilityLabel(sd, baseline) {
  if(sd === null || sd === undefined) return { lbl:'N/A', col:'var(--text-muted)' };
  const ratio = sd / baseline;
  if(ratio < 0.75) return { lbl:'STABLE ▼',  col:'var(--accent-teal)'  };
  if(ratio < 1.10) return { lbl:'NORMAL',     col:'var(--accent-blue)'  };
  if(ratio < 1.40) return { lbl:'VOLATILE ↑', col:'var(--accent-gold)'  };
  return             { lbl:'HIGH VOL ⚡',  col:'var(--accent-red)'   };
}

// Mini sparkline using inline divs (bar chart style, no canvas needed)
function miniSparkline(arr, color='var(--accent-blue)') {
  ...
  return `<div style="display:flex;align-items:flex-end;gap:2px;height:28px;">` +
    arr.map((v,i) => {
      ...
      return `<div title="Game ${i+1}: ${v}" style="flex:1;height:${h}px;background:${color};border-radius:2px 2px 0 0;opacity:${0.5 + (i/arr.length)*0.5};min-width:6px;"></div>`;
    }).join('') + `</div>`;
}
...
function renderVolatilityPanel(hS, aS, ht, at) {
  ...
  // Baseline σ values for labeling (Poisson theoretical at league avg)
  const BASE_GOALS   = 1.10;  // √1.2 ≈ 1.10 (league avg ~1.2 goals/game)
  const BASE_CORNERS = 2.26;  // √5.1
  const BASE_CARDS   = 1.48;  // √2.2

  const fmt = v => v !== null && v !== undefined ? Number(v).toFixed(2) : '—';

  const teamBlock = (label, r6, sea, isHome) => {
    const col = isHome ? 'var(--accent-blue)' : 'var(--accent-gold)';

    const gV  = volatilityLabel(r6.sdGoals,   BASE_GOALS);
    const gaV = volatilityLabel(r6.sdGoalsAgainst, BASE_GOALS);
    const cV  = volatilityLabel(r6.sdCorners,  BASE_CORNERS);
    const cdV = volatilityLabel(r6.sdCards,    BASE_CARDS);
    ...
    <div style="flex:1;min-width:220px;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:10px;">
        <div style="background:var(--bg-surface);border-radius:6px;padding:8px;">
          <div style="font-size:0.58rem;color:var(--text-muted);margin-bottom:2px;">Last 6 · σ</div>
          <div style="font-size:1.1rem;font-weight:900;font-family:'Fira Code',monospace;color:${gV.col};">${fmt(r6.sdGoals)}</div>
          <div style="font-size:0.58rem;font-weight:700;color:${gV.col};">${gV.lbl}</div>
        </div>
        <div style="background:var(--bg-surface);border-radius:6px;padding:8px;">
          <div style="font-size:0.58rem;color:var(--text-muted);margin-bottom:2px;">Season · σ</div>
          <div style="font-size:1.1rem;font-weight:900;font-family:'Fira Code',monospace;color:var(--text-main);">${fmt(sea.sdGoals)}</div>
          <div style="font-size:0.58rem;color:var(--text-muted);">n=${sea.n||'?'} αγώνες</div>
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:10px;">
        <div style="background:var(--bg-surface);border-radius:6px;padding:8px;">
          <div style="font-size:0.58rem;color:var(--text-muted);margin-bottom:2px;">Last 6 · σ</div>
          <div style="font-size:1.1rem;font-weight:900;font-family:'Fira Code',monospace;color:${gaV.col};">${fmt(r6.sdGoalsAgainst)}</div>
          <div style="font-size:0.58rem;font-weight:700;color:${gaV.col};">${gaV.lbl}</div>
        </div>
        <div style="background:var(--bg-surface);border-radius:6px;padding:8px;">
          <div style="font-size:0.58rem;color:var(--text-muted);margin-bottom:2px;">Season · σ</div>
          <div style="font-size:1.1rem;font-weight:900;font-family:'Fira Code',monospace;color:var(--text-main);">${fmt(sea.sdGoalsAgainst)}</div>
          <div style="font-size:0.58rem;color:var(--text-muted);">βάση sezόν</div>
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:10px;">
        <div style="background:var(--bg-surface);border-radius:6px;padding:8px;">
          <div style="font-size:0.58rem;color:var(--text-muted);margin-bottom:2px;">Last 6 · σ</div>
          <div style="font-size:1.1rem;font-weight:900;font-family:'Fira Code',monospace;color:${cV.col};">${fmt(r6.sdCorners)}</div>
          <div style="font-size:0.58rem;font-weight:700;color:${cV.col};">${cV.lbl}</div>
        </div>
        <div style="background:var(--bg-surface);border-radius:6px;padding:8px;">
          <div style="font-size:0.58rem;color:var(--text-muted);margin-bottom:2px;">Season · σ</div>
          <div style="font-size:1.1rem;font-weight:900;font-family:'Fira Code',monospace;color:var(--text-main);">${fmt(sea.sdCorners)}</div>
          <div style="font-size:0.58rem;color:var(--text-muted);">Poisson θεωρητικό</div>
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:10px;">
        <div style="background:var(--bg-surface);border-radius:6px;padding:8px;">
          <div style="font-size:0.58rem;color:var(--text-muted);margin-bottom:2px;">Last 6 · σ</div>
          <div style="font-size:1.1rem;font-weight:900;font-family:'Fira Code',monospace;color:${cdV.col};">${fmt(r6.sdCards)}</div>
          <div style="font-size:0.58rem;font-weight:700;color:${cdV.col};">${cdV.lbl}</div>
        </div>
        <div style="background:var(--bg-surface);border-radius:6px;padding:8px;">
          <div style="font-size:0.58rem;color:var(--text-muted);margin-bottom:2px;">Season · σ</div>
          <div style="font-size:1.1rem;font-weight:900;font-family:'Fira Code',monospace;color:var(--text-main);">${fmt(sea.sdCards)}</div>
          <div style="font-size:0.58rem;color:var(--text-muted);">Poisson θεωρητικό</div>
        </div>
      </div>
  ...
  // Combined volatility signal: if BOTH teams high-vol in goals → warn prediction reliability
  const hSdG = hr6.sdGoals; const aSdG = ar6.sdGoals;
  ...
  const combinedAlert = bothVolatile
    ? `<div style="background:rgba(244,63,94,0.08);border:1px solid rgba(244,63,94,0.3);border-radius:6px;padding:8px 12px;font-size:0.7rem;margin-bottom:14px;">⚠️ <strong>Υψηλή αμοιβαία διακύμανση</strong> — Και οι δύο ομάδες παρουσιάζουν ασταθή αποτελέσματα (last 6). Η πρόβλεψη έχει μεγαλύτερο εύρος αβεβαιότητας.</div>`
    : bothStable
    ? `<div style="background:rgba(16,185,129,0.08);border:1px solid rgba(16,185,129,0.3);border-radius:6px;padding:8px 12px;font-size:0.7rem;margin-bottom:14px;">✅ <strong>Σταθερές αποδόσεις και από τις δύο ομάδες</strong> — Υψηλότερη αξιοπιστία πρόβλεψης. Χαμηλή διακύμανση last 6.</div>`
    : '';

  return combinedAlert + `<div style="display:flex;gap:20px;flex-wrap:wrap;">${teamBlock(ht, hr6, hSea, true)}${teamBlock(at, ar6, aSea, false)}</div>`;
}
...
function getMatchCardHTML(d) {
  ...
  <div class="match-card" id="card-${d.fixId}">
    ...
    <div id="details-${d.fixId}" style="display:none; margin-top:20px; padding-top:20px; border-top:1px dashed var(--border-light);">
      ...
      <div class="stat-box" style="margin-top:12px;">
        <div class="stat-box-title">📉 Volatility Analysis — Διακύμανση Αποδόσεων</div>
        ${renderVolatilityPanel(d.hS, d.aS, d.ht, d.at)}
      </div>
