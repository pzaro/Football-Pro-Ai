...
// v2.2 - FIX: Over 3.5 consistency guard (bestScore vs pick),
//         corner analysis on every match, Poisson O3.5 UI display
...
/**
 * Corner Analysis — v2.2
 * Παράγει ανάλυση κορνέρ για ΚΑΘΕ ματς (ανεξάρτητα από το omegaPick).
 * Επιστρέφει { total, hCors, aCors, signal, detail, overLine, confidence }
 */
function analyzeCorners(hCors, aCors, hSplitXG, aSplitXG, hFormXG, aFormXG) {
  const total    = Number(hCors || 0) + Number(aCors || 0);
  const hC       = Number(hCors || 0);
  const aC       = Number(aCors || 0);

  // Χρησιμοποιούμε xG ως proxy για attacking intent → περισσότερο xG = περισσότερα κορνέρ
  // split xG είναι πιο αξιόπιστο (home/away specific)
  const hXGFactor = (Number(hSplitXG || 0) + Number(hFormXG || 0)) / 2;
  const aXGFactor = (Number(aSplitXG || 0) + Number(aFormXG || 0)) / 2;

  // Ποσοστό κορνέρ home vs away
  const totalSafe = Math.max(total, 0.1);
  const hPct = ((hC / totalSafe) * 100).toFixed(0);
  const aPct = ((aC / totalSafe) * 100).toFixed(0);

  // Over line βάσει total
  let overLine, signal, confidence, signalColor;

  if (total >= 13.5) {
    overLine = "OVER 12.5"; confidence = "HIGH"; signalColor = "var(--accent-green)";
    signal = `🚩 ${overLine} CORNERS`;
  } else if (total >= 11.5) {
    overLine = "OVER 10.5"; confidence = "MED-HIGH"; signalColor = "var(--accent-green)";
    signal = `🚩 ${overLine} CORNERS`;
  } else if (total >= 9.5) {
    overLine = "OVER 8.5"; confidence = "MEDIUM"; signalColor = "var(--accent-gold)";
    signal = `🚩 ${overLine} CORNERS`;
  } else if (total <= 7.5) {
    overLine = "UNDER 8.5"; confidence = "MEDIUM"; signalColor = "var(--accent-teal)";
    signal = `🔒 ${overLine} CORNERS`;
  } else {
    overLine = "~9-10"; confidence = "LOW"; signalColor = "var(--text-muted)";
    signal = `📊 NEUTRAL (~${total.toFixed(1)} exp.)`;
  }

  const detail = `Home: ${hC.toFixed(1)} (${hPct}%) | Away: ${aC.toFixed(1)} (${aPct}%) | Total Exp: ${total.toFixed(1)}`;

  return { total, hCors: hC, aCors: aC, signal, detail, overLine, confidence, signalColor };
}
// ================================================================
/**
...
function computePick(hXG, aXG, tXG, bttsScore, cor, totCards, lp, hS, aS, h2h) {
  ...
  // FIX #8 + v2.2 CONSISTENCY GUARD:
  // Αν το bestScore δεν έχει >= 3 γκολ συνολικά ή τα lambdas δεν αθροίζουν > 3.5,
  // το Over 3.5 είναι εσωτερικά αντιφατικό — το απαγορεύουμε.
  const lambdaTotal = hLambda + aLambda;
  const bestScoreTotal = (pp.bestScore?.h ?? 0) + (pp.bestScore?.a ?? 0);

  if (pp.pO35 >= 0.44 && tXG >= lp.minXGO35 && btts >= 1.15 && (hForm + aForm) / 2 >= 1.05
      && lambdaTotal > 3.50 && bestScoreTotal >= 3) {
    omegaPick = "🚀 OVER 3.5 GOALS";
    pickScore = (pp.pO35 + h2hBoost) * 100;
    reason = `Poisson O3.5: ${(pp.pO35*100).toFixed(1)}% | tXG: ${tXG.toFixed(2)} | λ: ${lambdaTotal.toFixed(2)} | Form: ✓`;
  }
  ...
  return { omegaPick, reason, pickScore, outPick, hG, aG, hExp, aExp, exactConf, xgDiff, pp, lambdaTotal };
}
