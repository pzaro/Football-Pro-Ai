...
const LS_POSTMATCH_RANGE = "omega_postmatch_range_v5.0";
const POSTMATCH_RANGES = new Set(["CURRENT","7","30","90","ALL"]);
let postMatchRange = "CURRENT";
let postMatchSyncing = false;
try {
  const _pmSaved = localStorage.getItem(LS_POSTMATCH_RANGE);
  if(POSTMATCH_RANGES.has(_pmSaved)) postMatchRange = _pmSaved;
} catch {}
...
const todayISO = () => {
  const p=new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Athens',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date());
...
function getDatesInRange(s,e){
  const d=[];
  let c=new Date(`${s}T00:00:00Z`),end=new Date(`${e}T00:00:00Z`);
  while(c<=end){d.push(c.toISOString().split('T')[0]);c.setUTCDate(c.getUTCDate()+1);}
  return d;
}
...
function setBtnsDisabled(d){
  ['btnPre','btnSyncLive','scanLeagueAllBtn','scanLeagueNoneBtn','scanLeagueRefreshBtn','selfImproveNowBtn','scanMode','scanStart','scanStartTime','scanEnd','scanEndTime'].forEach(id=>{const el=document.getElementById(id);if(el)el.disabled=d;});
  document.querySelectorAll('#scanLeagueCheckboxes input[type="checkbox"]').forEach(el=>{el.disabled=d;});
  if(!d && (document.getElementById('scanMode')?.value||'REMAINING')==='REMAINING'){
    const sd=document.getElementById('scanStart'),st=document.getElementById('scanStartTime');
    if(sd)sd.disabled=true;if(st)st.disabled=true;
  }
}
...
const PREMATCH_SCAN_STATUS = new Set(['NS','TBD']);
const NON_SCANNABLE_STATUS = new Set(['PST','CANC','ABD','AWD','WO']);
const INTERRUPTED_STATUS = new Set(['SUSP','INT']);
function _greeceDateTimeParts(date=new Date()){
  const parts=new Intl.DateTimeFormat('en-GB',{
    timeZone:GREECE_TIME_ZONE,year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hourCycle:'h23'
  }).formatToParts(date);
  return Object.fromEntries(parts.filter(p=>p.type!=='literal').map(p=>[p.type,p.value]));
}
function greeceNowHHMM(){const p=_greeceDateTimeParts();return `${p.hour}:${p.minute}`;}
function _athensWallTimeToDate(dateStr,timeStr='00:00'){
  const dm=String(dateStr||'').match(/^(\d{4})-(\d{2})-(\d{2})$/);
  const tm=String(timeStr||'00:00').match(/^(\d{2}):(\d{2})/);
  if(!dm||!tm) return null;
  const target=Date.UTC(+dm[1],+dm[2]-1,+dm[3],+tm[1],+tm[2],0,0);
  let guess=target;
  // Iterative conversion of an Athens wall-clock time to the matching UTC instant.
  for(let i=0;i<3;i++){
    const p=_greeceDateTimeParts(new Date(guess));
...
function _formatAthensWindowDate(d){
  if(!(d instanceof Date)||Number.isNaN(d.getTime())) return '—';
  return d.toLocaleString('el-GR',{timeZone:GREECE_TIME_ZONE,day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',hour12:false});
}
function _getScanWindow({strict=false}={}){
  const mode=document.getElementById('scanMode')?.value||'REMAINING';
  const now=new Date();
  const nowP=_greeceDateTimeParts(now);
  const autoStartDate=`${nowP.year}-${nowP.month}-${nowP.day}`;
  const autoStartTime=`${nowP.hour}:${nowP.minute}`;
  const startDate=mode==='REMAINING'?autoStartDate:(document.getElementById('scanStart')?.value||autoStartDate);
  const startTime=mode==='REMAINING'?autoStartTime:(document.getElementById('scanStartTime')?.value||'00:00');
  const endDate=document.getElementById('scanEnd')?.value||startDate;
  const endTime=document.getElementById('scanEndTime')?.value||'23:59';
  const from=mode==='REMAINING'?now:_athensWallTimeToDate(startDate,startTime);
  const to=_athensWallTimeToDate(endDate,endTime);
  let error='';
  if(!from||!to) error='Ελέγξτε την ημερομηνία και την ώρα του scanner.';
  else if(to.getTime()<=from.getTime()) error='Η τελική ημερομηνία/ώρα πρέπει να είναι μετά την έναρξη του παραθύρου.';
  if(strict&&error) throw new Error(error);
  return {mode,from,to,error,fromDate:startDate,fromTime:startTime,toDate:endDate,toTime:endTime};
}
function _fixtureKickoffMs(m){
  const d=new Date(m?.fixture?.date||'');
  return Number.isNaN(d.getTime())?NaN:d.getTime();
}
function _fixtureInScanWindow(m,win){
  const t=_fixtureKickoffMs(m);
  return Number.isFinite(t)&&win?.from&&win?.to&&t>=win.from.getTime()&&t<=win.to.getTime();
}
function _scanFixtureBucket(m){
  const s=String(m?.fixture?.status?.short||'').toUpperCase();
  if(PREMATCH_SCAN_STATUS.has(s) || (!s && _fixtureKickoffMs(m)>Date.now())) return 'prematch';
  if(isLive(s)||INTERRUPTED_STATUS.has(s)) return 'live';
  if(isFinished(s)) return 'finished';
  if(NON_SCANNABLE_STATUS.has(s)) return 'unavailable';
  return 'other';
}
function _scanWindowKey(win){
  if(!win) return '';
  return win.mode==='REMAINING'
    ? `REMAINING|${win.toDate||''}|${win.toTime||''}`
    : `CUSTOM|${win.fromDate||''}|${win.fromTime||''}|${win.toDate||''}|${win.toTime||''}`;
}
function _renderScanWindowSummary(win,summary=null){
  const el=document.getElementById('scanWindowSummary');
  if(!el) return;
  if(!win||win.error){
    el.className='scan-window-summary error';
    el.innerHTML=`<span class="scan-window-icon">⚠</span><span>${esc(win?.error||'Μη έγκυρο χρονικό παράθυρο.')}</span>`;
    return;
  }
  const modeTxt=win.mode==='REMAINING'?'ΥΠΟΛΟΙΠΟΙ ΑΓΩΝΕΣ · από τώρα':'CUSTOM RANGE';
  const counts=summary?` · <b>${summary.prematch||0}</b> προς ανάλυση · ${summary.live||0} live · ${summary.finished||0} ολοκληρωμένα · ${summary.unavailable||0} μη διαθέσιμα`:'';
  el.className='scan-window-summary';
  el.innerHTML=`<span class="scan-window-icon">🕒</span><span><b>${modeTxt}</b> · ${_formatAthensWindowDate(win.from)} → ${_formatAthensWindowDate(win.to)} · ώρα Ελλάδος${counts}</span>`;
}
window.onScanModeChange=function(refresh=true){
  const mode=document.getElementById('scanMode')?.value||'REMAINING';
  const sd=document.getElementById('scanStart'),st=document.getElementById('scanStartTime');
  const p=_greeceDateTimeParts();
  if(mode==='REMAINING'){
    if(sd){sd.value=`${p.year}-${p.month}-${p.day}`;sd.disabled=true;}
    if(st){st.value=`${p.hour}:${p.minute}`;st.disabled=true;}
  }else{
    if(sd){if(!sd.value)sd.value=`${p.year}-${p.month}-${p.day}`;sd.disabled=false;}
    if(st){if(!st.value)st.value=`${p.hour}:${p.minute}`;st.disabled=false;}
  }
  _renderScanWindowSummary(_getScanWindow({strict:false}),_scanPreview?.summary||null);
  scheduleScanLeagueRefresh();
};
function _displayRecord(rec){
  if(!rec) return null;
  if(rec.m?.fixture) return rec;
  const id=String(rec.fixId??rec.fixtureId??'');
  return (window.scannedMatchesData||[]).find(x=>String(x?.fixId??x?.m?.fixture?.id??'')===id) || rec;
}
function formatGreeceKickoff(rec,{withDate=false}={}){
  const src=_displayRecord(rec)||rec;
  const raw=src?.m?.fixture?.date || rec?.fixtureDate || rec?.date || null;
  if(!raw) return '—';
  const d=new Date(raw);
  if(Number.isNaN(d.getTime())) return String(raw);
  const opts={timeZone:GREECE_TIME_ZONE,hour:'2-digit',minute:'2-digit',hour12:false};
  if(withDate){opts.day='2-digit';opts.month='2-digit';}
  return d.toLocaleString('el-GR',opts);
}
function _fixtureState(rec){
  const src=_displayRecord(rec)||rec;
  const status=src?.m?.fixture?.status?.short || rec?.status || rec?.pmStatus || '';
  const home=src?.m?.goals?.home ?? rec?.pmActualHome;
  const away=src?.m?.goals?.away ?? rec?.pmActualAway;
  return {src,status,home,away,finished:isFinished(status),live:isLive(status)};
}
function matchLifecycleHTML(rec,{compact=false}={}){
  const st=_fixtureState(rec);
  const time=formatGreeceKickoff(rec,{withDate:!compact});
  if(st.finished){
    const score=(st.home!==null&&st.home!==undefined&&st.away!==null&&st.away!==undefined)?`${st.home}–${st.away}`:'—';
    return `<span class="match-completed-badge">✓ ΟΛΟΚΛΗΡΩΘΗΚΕ</span><span class="match-final-score-badge">FT ${score}</span><span class="match-greece-time" title="Ώρα έναρξης Ελλάδος">🇬🇷 ${time}</span>`;
  }
  if(st.live){
    const score=(st.home!==null&&st.home!==undefined&&st.away!==null&&st.away!==undefined)?`${st.home}–${st.away}`:'0–0';
    return `<span class="match-live-badge">● LIVE ${score}</span><span class="match-greece-time" title="Ώρα έναρξης Ελλάδος">🇬🇷 ${time}</span>`;
  }
  return `<span class="match-greece-time" title="Ώρα έναρξης Ελλάδος">🕒 🇬🇷 ${time}</span>`;
}
function competitionContextHTML(rec,{compact=false,showLifecycle=true}={}){
  if(!rec) return '';
  const src=_displayRecord(rec)||rec;
  const lg=rec.lg||src?.lg||src?.m?.league?.name||'—';
  const cup=!!(rec.isCup||src?.isCup||isCupCompetitionMeta(rec.leagueId||src?.leagueId||src?.m?.league?.id,lg));
  const h=rec.homeCategory||src?.homeCategory||src?.hS?.primaryCompetition||null;
  const a=rec.awayCategory||src?.awayCategory||src?.aS?.primaryCompetition||null;
  ...
  if(showLifecycle) html+=matchLifecycleHTML(src,{compact});
  if(cup){
    html+=`<span class="competition-cup-badge">CUP</span>`;
    html+=`<span class="competition-team-level" title="Κατηγορία ${esc(rec.ht||src?.ht||'γηπεδούχου')}">🏠 ${h?.name?esc(h.name):'Κατηγορία μη διαθέσιμη'}</span>`;
    html+=`<span class="competition-team-level" title="Κατηγορία ${esc(rec.at||src?.at||'φιλοξενούμενου')}">✈️ ${a?.name?esc(a.name):'Κατηγορία μη διαθέσιμη'}</span>`;
  }
  return html+`</div>`;
}

// v5.8 — frozen list of model-confirmed markets + post-match settlement.
function collectConfirmedMarketSignals(rec){
  if(!rec) return [];
  if(Array.isArray(rec.preConfirmedSignals) && rec.preConfirmedSignals.length) return rec.preConfirmedSignals;
  const out=[], seen=new Set();
  const add=(key,label,market,confidence,extra={})=>{
    if(!key||seen.has(key)) return; seen.add(key);
    out.push({key,label,market,confidence:Number.isFinite(Number(confidence))?Number(confidence):null,...extra});
  };
  const pp=rec.pp||{};
  const mp=rec.matchPicture||null;
  const lp=typeof getLeagueParams==='function'?getLeagueParams(rec.leagueId||rec.m?.league?.id):{};
  const tXG=Number(rec.tXG ?? ((Number(rec.hXGfinal)||0)+(Number(rec.aXGfinal)||0)));
  const hXG=Number(rec.hXGfinal||rec.hExp||0), aXG=Number(rec.aXGfinal||rec.aExp||0);
  const market=mp?.recommendedMarket||rec.outcomeMarket;
  const marketProb=Number(mp?.recommendedProb??rec.outcomeProb??0);
  if(['1','X','2','1X','X2','12'].includes(market) && marketProb>=0.65) add(`OUT_${market}`,`Σημείο ${market}`,'1X2 / DC',marketProb*100,{selection:market});
  if(Number(pp.pO35)>=0.52 && tXG>=Number(lp.minXGO35??3.8)) add('O35','Over 3.5','Γκολ',Number(pp.pO35)*100);
  if(Number(pp.pO25)>=0.62 && tXG>=Number(lp.minXGO25??3.1)) add('O25','Over 2.5','Γκολ',Number(pp.pO25)*100);
  if(Number(pp.pU25)>=0.58 && tXG<=Number(lp.maxU25??1.8)) add('U25','Under 2.5','Γκολ',Number(pp.pU25)*100);
  if(Number(pp.pBTTS)>=0.68 && hXG>=1.10 && aXG>=1.10) add('BTTS','GG','BTTS',Number(pp.pBTTS)*100);
  if(Number(rec.cornerConf)>=72) add('COR85','Over 8.5','Κόρνερ',Number(rec.cornerConf));
  const cards=Number(rec.hS?.crd||0)+Number(rec.aS?.crd||0);
  if(cards>=Number(engineConfig?.minCards??6.1) && Math.abs(Number(rec.xgDiff||0))<0.45) add('CRD55','Over 5.5','Κάρτες',Math.min(99,Math.max(0,(cards-5)*20)));
  if(rec.exact && Number(rec.exactConf||0)>=70) add('EXACT','Ακριβές σκορ','Σκορ',Number(rec.exactConf),{scores:[rec.exact,rec.exact2].filter(Boolean)});
  if(rec.htAnalysis?.htBest && Number(rec.htAnalysis?.htConf||0)>=70) add('HTEXACT','Σκορ ΗΤ','Ημίχρονο',Number(rec.htAnalysis.htConf),{scores:[`${rec.htAnalysis.htBest.h}-${rec.htAnalysis.htBest.a}`,rec.htAnalysis.htSecond?`${rec.htAnalysis.htSecond.h}-${rec.htAnalysis.htSecond.a}`:null].filter(Boolean)});
  if(Number(rec.offside?.pTotOff35)>=70) add('OFF35','Over 3.5','Οφσάιντ',Number(rec.offside.pTotOff35));
  // Ensure the primary signal is represented even when it has a special wording.
  const pick=String(rec.omegaPick||'');
  if(pick && !pick.includes('ΧΩΡΙΣ') && !pick.includes('NO BET')){
    if(pick.includes('ΚΟΡΝΕΡ')) add('COR85','Over 8.5','Κόρνερ',rec.cornerConf);
    else if(pick.includes('ΚΑΡΤΕΣ')) add('CRD55','Over 5.5','Κάρτες',rec.strength);
    else if(pick.includes('ΠΑΝΩ ΑΠΟ 3.5')) add('O35','Over 3.5','Γκολ',rec.strength);
    else if(pick.includes('ΠΑΝΩ ΑΠΟ 2.5')) add('O25','Over 2.5','Γκολ',rec.strength);
    else if(pick.includes('ΚΑΤΩ ΑΠΟ 2.5')) add('U25','Under 2.5','Γκολ',rec.strength);
    else if(pick.includes('ΓΚΟΛ/ΓΚΟΛ')||pick.includes('GG')) add('BTTS','GG','BTTS',rec.strength);
  }
  return out;
}
function settleConfirmedMarketSignal(rec,sig){
  const st=_fixtureState(rec); if(!st.finished) return null;
  const h=Number(st.home), a=Number(st.away); if(!Number.isFinite(h)||!Number.isFinite(a)) return null;
  const total=h+a, outcome=h>a?'1':h<a?'2':'X';
  const act=rec.actStats||null;
  switch(sig.key){
    case 'OUT_1': return outcome==='1'; case 'OUT_X': return outcome==='X'; case 'OUT_2': return outcome==='2';
    case 'OUT_1X': return outcome==='1'||outcome==='X'; case 'OUT_X2': return outcome==='X'||outcome==='2'; case 'OUT_12': return outcome==='1'||outcome==='2';
    case 'O25': return total>=3; case 'O35': return total>=4; case 'U25': return total<=2; case 'BTTS': return h>0&&a>0;
    case 'COR85': {const v=act&&_pmHasNum(act.hCor)&&_pmHasNum(act.aCor)?Number(act.hCor)+Number(act.aCor):null; return v===null?null:v>=9;}
    case 'CRD55': {const v=act&&_pmHasNum(act.hCrd)&&_pmHasNum(act.aCrd)?Number(act.hCrd)+Number(act.aCrd):null; return v===null?null:v>=6;}
    case 'OFF35': {const v=act&&_pmHasNum(act.hOff)&&_pmHasNum(act.aOff)?Number(act.hOff)+Number(act.aOff):null; return v===null?null:v>=4;}
    case 'EXACT': {const sc=`${h}-${a}`; return (sig.scores||[]).includes(sc);}
    case 'HTEXACT': {
      const hh=rec.m?.score?.halftime?.home, ha=rec.m?.score?.halftime?.away;
      if(!_pmHasNum(hh)||!_pmHasNum(ha)) return null;
      return (sig.scores||[]).includes(`${Number(hh)}-${Number(ha)}`);
    }
    default:return null;
  }
}
function confirmedMarketsSettlementHTML(rec,{compact=false}={}){
  const sigs=collectConfirmedMarketSignals(rec);
  if(!sigs.length) return `<span class="settled-signal-chip neutral">Δεν υπήρχαν επιβεβαιωμένα markets</span>`;
  return `<div class="settled-market-grid ${compact?'compact':''}">${sigs.map(sig=>{
    const hit=settleConfirmedMarketSignal(rec,sig);
    const cls=hit===true?'won':hit===false?'lost':'unknown';
...
// ================================================================
//  TIME-WINDOW LEAGUE SELECTOR — ATHENS TIME + STATUS GUARD
//  Preview counts all fixtures in the chosen time window, but exposes
//  only true pre-match fixtures (NS/TBD) to the heavy analysis engine.
// ================================================================
let _scanPreview = { key:'', fixtures:[], eligibleFixtures:[], leagues:[], summary:null, window:null, loadedAt:0 };
...
function _renderScanLeagueSelector(leagues, {loading=false,error='',resetSelection=false,summary=null}={}){
  const box=document.getElementById('scanLeagueCheckboxes');
  const status=document.getElementById('scanLeagueSelectorStatus');
  if(!box||!status) return;
  if(loading){ status.textContent='⏳ Φόρτωση αγώνων στο επιλεγμένο χρονικό παράθυρο…'; box.innerHTML=''; return; }
  if(error){ status.textContent=`⚠ ${error}`; box.innerHTML=''; return; }
  if(!leagues?.length){
    const s=summary||_scanPreview.summary||{};
    status.textContent=`Δεν υπάρχουν pre-match αγώνες προς ανάλυση · ${s.live||0} live · ${s.finished||0} ολοκληρωμένα · ${s.unavailable||0} μη διαθέσιμα`;
    box.innerHTML='';
    return;
  }
  ...
  const selectedMatches=leagues.filter(x=>_scanSelectedLeagueIds.has(Number(x.id))).reduce((n,x)=>n+x.matches,0);
  const s=summary||_scanPreview.summary||{};
  status.textContent=`${selectedCount}/${leagues.length} πρωταθλήματα · ${selectedMatches} προς ανάλυση · ${s.live||0} live · ${s.finished||0} ολοκληρωμένα · ${s.unavailable||0} μη διαθέσιμα`;
...
window.toggleScanLeague=function(id,checked){
  ...
  _renderScanLeagueSelector(_scanPreview.leagues,{resetSelection:false,summary:_scanPreview.summary});
};
...
async function _loadScanFixturePreview(startD,endD,{force=false}={}){
  const win=_getScanWindow({strict:false});
  _renderScanWindowSummary(win,_scanPreview.summary);
  if(win.error){
    _renderScanLeagueSelector([],{error:win.error});
    return {key:'',fixtures:[],eligibleFixtures:[],leagues:[],summary:null,window:win,loadedAt:0};
  }
  startD=win.fromDate; endD=win.toDate;
  const key=_scanWindowKey(win);
  // Remaining mode moves with the clock; cached previews older than 45s are refreshed.
  const cacheFresh=Date.now()-Number(_scanPreview.loadedAt||0)<45000;
  if(!force && _scanPreview.key===key && _scanPreview.fixtures.length && cacheFresh){
    _renderScanLeagueSelector(_scanPreview.leagues,{resetSelection:false,summary:_scanPreview.summary});
    _renderScanWindowSummary(win,_scanPreview.summary);
    return _scanPreview;
  }
  ...
  try{
    ...
    for(const date of getDatesInRange(startD,endD)){
      ...
      if(seq!==_scanPreviewSeq) return _scanPreview;
      (res?.response||[]).forEach(m=>{ if(supported.has(Number(m.league?.id))) raw.push(m); });
      if(raw.length>1200) break; // hard safety cap before window filtering
    }
    const fixtures=raw.filter(m=>_fixtureInScanWindow(m,win)).slice(0,600);
    const summary={prematch:0,live:0,finished:0,unavailable:0,other:0,total:fixtures.length};
    const eligible=[];
    fixtures.forEach(m=>{
      const bucket=_scanFixtureBucket(m);
      summary[bucket]=(summary[bucket]||0)+1;
      if(bucket==='prematch') eligible.push(m);
    });
    const capped=eligible.slice(0,350);
    const counts=new Map();
    capped.forEach(m=>{
    ...
    _scanPreview={key,fixtures,eligibleFixtures:capped,leagues,summary,window:win,loadedAt:Date.now()};
    _renderScanLeagueSelector(leagues,{resetSelection:changedKey,summary});
    _renderScanWindowSummary(win,summary);
    return _scanPreview;
  }catch(e){
    console.warn('[APEX] time-window preview:',e.message);
...
window.refreshScanLeagueOptions=async function(force=false){
  const win=_getScanWindow({strict:false});
  _renderScanWindowSummary(win,_scanPreview.summary);
  if(win.error){_renderScanLeagueSelector([],{error:win.error});return _scanPreview;}
  return _loadScanFixturePreview(win.fromDate,win.toDate,{force});
};
...
window.runScan=async function(){
  if(isRunning)return;
  let scanWindow;
  try{scanWindow=_getScanWindow({strict:true});}catch(e){showErr(e.message);return;}
  const startD=scanWindow.fromDate, endD=scanWindow.toDate;
  ...
  // Preserve advisorSection: it hosts the independent Live Tracker and Bet Journal.
  ['topSection','summarySection','auditSection'].forEach(id=>{const el=document.getElementById(id);if(el)el.innerHTML='';});
  ...
  try{
    ...
    // Force a fresh preview at Run Scan so a match that has already kicked off
    // cannot slip into the pre-match analysis because of stale status data.
    const preview=await _loadScanFixturePreview(startD,endD,{force:true});
    const selectedIds=new Set(window.getScanSelectedLeagueIds().map(Number));
    if(!preview.leagues.length){showErr('Δεν υπάρχουν pre-match αγώνες (NS/TBD) προς ανάλυση στο επιλεγμένο χρονικό παράθυρο.');return;}
    if(!selectedIds.size){showErr('Επιλέξτε τουλάχιστον ένα πρωτάθλημα πριν από το Run Scan.');return;}
    let all=(preview.eligibleFixtures||[]).filter(m=>selectedIds.has(Number(m.league.id))).filter(m=>_scanFixtureBucket(m)==='prematch').slice(0,350);
    if(!all.length){showErr('Δεν απομένουν pre-match αγώνες στα πρωταθλήματα που έχετε τσεκάρει.');return;}
    ...
    if(fallbackCount > 0) {
      ...
      showOk(`✅ Scan ολοκληρώθηκε — ${all.length} pre-match αγώνες · ${_formatAthensWindowDate(scanWindow.from)} → ${_formatAthensWindowDate(scanWindow.to)}.`);
    }
  ...
  try{
    ...
    for(const lf of all){
      ...
      if(prev&&inPlay&&prev.inPlayPick!==inPlay.inPlayPick){
        const alert={time:new Date().toLocaleTimeString('el-GR',{timeZone:GREECE_TIME_ZONE,hour:'2-digit',minute:'2-digit'}),fixId,ht:lf.teams.home.name,at:lf.teams.away.name,elapsed:lf.fixture.status.elapsed,from:prev.inPlayPick,to:inPlay.inPlayPick,score:`${lf.goals.home}-${lf.goals.away}`};
    ...
    if(lastEl)lastEl.textContent=new Date().toLocaleTimeString('el-GR',{timeZone:GREECE_TIME_ZONE,hour:'2-digit',minute:'2-digit'});
    ...
    return`<div class="match-card${isStrong?' live-strong-signal':''}" id="live-card-${lf.fixture.id}" style="border-color:${cardBorder};${isStrong?'border-width:2px;':''}">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;">
        <div style="flex:1;min-width:160px;">
          <div class="match-league"><span class="live-dot"></span><span class="league-badge">${esc(status)}</span><span style="color:var(--text-muted);font-size:0.65rem;margin-left:4px;">${esc(lf.league.name)}</span><span class="match-greece-time">🇬🇷 ${formatGreeceKickoff({m:lf},{withDate:false})}</span></div>
...
function _drawerRecord(fixId){
  ...
  const active=(window.scannedMatchesData||[]).find(x=>String(x?.fixId??x?.m?.fixture?.id??'')===id);
  if(active) return active;
  try{
    const v=_pmVault().find(x=>String(x?.fixtureId??'')===id && _pmParseScore(x?.pmActualScore));
    return v?_pmVaultToMatch(v):null;
  }catch{return null;}
}
function _drawerBodyFromAccordion(rec){
  if(!rec) return '';
  if(rec._fromVault){
    return `<div class="accordion-card" style="margin:0;"><h4>🏁 Αποτελέσματα & επιβεβαιώσεις</h4><div style="padding:10px 12px;">${confirmedMarketsSettlementHTML(rec)}</div></div>`;
  }
  if(typeof buildAccordionHTML!=='function') return '';
...
window.openMatchAnalysisDrawer=function(fixId){
  ...
  if(!rec){ showErr('Δεν βρέθηκε αποθηκευμένη ανάλυση για τον συγκεκριμένο αγώνα.'); return; }
  _ensureMatchAnalysisDrawer();
  const sh=rec.m?.fixture?.status?.short||'';
  const live=typeof isLive==='function' && isLive(sh);
  const finished=isFinished(sh);
  const when=formatGreeceKickoff(rec,{withDate:true});
  const ph=Number(rec.pp?.pHome||rec.prePHome||0)*100, px=Number(rec.pp?.pDraw||rec.prePDraw||0)*100, pa=Number(rec.pp?.pAway||rec.prePAway||0)*100;
  const hXG=Number(rec.hXGfinal||0), aXG=Number(rec.aXGfinal||0), tXG=Number(rec.tXG ?? (hXG+aXG));
  const conf=Number(rec.strength||0);
  const finalScore=finished?`${rec.m?.goals?.home??rec.pmActualHome??'—'}–${rec.m?.goals?.away??rec.pmActualAway??'—'}`:null;
  const score=live ? ` · LIVE ${rec.m?.goals?.home??0}-${rec.m?.goals?.away??0}` : finished?` · FT ${finalScore}`:'';
  const liveBadge=live?'<span class="match-analysis-live-badge">● LIVE</span>':'';
  const completedBadge=finished?'<span class="match-analysis-completed-badge">✓ ΟΛΟΚΛΗΡΩΘΗΚΕ</span>':'';

  document.getElementById('matchDrawerLeague').innerHTML=`${liveBadge}${completedBadge}<span>ΑΝΑΛΥΣΗ ΑΓΩΝΑ</span>`;
  ...
  document.getElementById('matchDrawerMeta').innerHTML=`<span>🇬🇷 ${when} · ώρα Ελλάδος${score}</span>${competitionContextHTML(rec,{compact:false,showLifecycle:false})}`;
  ...
  document.getElementById('matchDrawerQuick').innerHTML=`
    ...
    ${finished?`<div class="match-analysis-settled"><div class="match-analysis-settled-title">ΕΠΙΒΕΒΑΙΩΜΕΝΕΣ ΠΡΟΤΑΣΕΙΣ · ΤΕΛΙΚΗ ΕΚΒΑΣΗ</div>${confirmedMarketsSettlementHTML(rec,{compact:true})}</div>`:''}`;
...
function buildAccordionHTML(x) {
  ...
  return `
    <td colspan="9" style="padding: 20px; text-align:left; border-bottom:1px solid var(--border-light); background:var(--bg-panel);">
      ...
      <!-- ΓΡΑΜΜΗ 2: Τα υπόλοιπα στοιχισμένα -->
...
// ================================================================
//  POST-MATCH HISTORY — read-only analytics over current scan / Vault
//  IMPORTANT: uses pm* fields so it NEVER feeds Auto-Calibration.
// ================================================================
function _pmHasNum(v){
  return v !== null && v !== undefined && v !== '' && Number.isFinite(Number(v));
}
function _pmVault(){
  try { const a=JSON.parse(localStorage.getItem(LS_PREDS)||'[]'); return Array.isArray(a)?a:[]; }
  catch { return []; }
}
function _pmDateUTC(v){
  const m=String(v||'').slice(0,10).match(/^(\d{4})-(\d{2})-(\d{2})$/);
  return m ? Date.UTC(Number(m[1]),Number(m[2])-1,Number(m[3])) : NaN;
}
function _pmTodayUTC(){
  const d=new Date();
  return Date.UTC(d.getFullYear(),d.getMonth(),d.getDate());
}
function _pmRangeLabel(v){
  return v==='CURRENT'?'Current Scan':v==='ALL'?'All Vault':`${v} ημέρες`;
}
function _pmRecordsForRange(range){
  const store=_pmVault();
  if(range==='CURRENT') return [];
  const today=_pmTodayUTC();
  const days=range==='ALL'?null:Math.max(1,parseInt(range,10)||7);
  const from=days===null?null:today-(days-1)*86400000;
  return store.filter(r=>{
    const t=_pmDateUTC(r.date);
    if(!Number.isFinite(t) || t>today) return false;
    return from===null || t>=from;
  }).sort((a,b)=>(_pmDateUTC(b.date)||0)-(_pmDateUTC(a.date)||0));
}
function _pmParseScore(v){
  const m=String(v||'').match(/^(\d+)\s*-\s*(\d+)$/);
  return m ? {h:Number(m[1]),a:Number(m[2])} : null;
}
function _pmVaultToMatch(v){
  const sc=_pmParseScore(v.pmActualScore);
  if(!sc) return null;
  let hPred=_pmHasNum(v.pmHXG)?Number(v.pmHXG):null;
  let aPred=_pmHasNum(v.pmAXG)?Number(v.pmAXG):null;
  // Legacy Vault compatibility: reconstruct xG split only when tXG + xgDiff exist.
  if((hPred===null || aPred===null) && _pmHasNum(v.tXG) && _pmHasNum(v.xgDiff)){
    const t=Number(v.tXG), d=Number(v.xgDiff);
    hPred=Math.max(0,(t+d)/2); aPred=Math.max(0,(t-d)/2);
  }
  return {
    _fromVault:true,
    _pmSchema:Number(v.pmSchema||0),
    fixId:v.fixtureId,
    ht:v.homeTeam||'—', at:v.awayTeam||'—', lg:v.league||'', leagueId:v.leagueId,
    m:{
      fixture:{id:v.fixtureId,date:v.date,status:{short:'FT'}},
      goals:{home:sc.h,away:sc.a},
      teams:{home:{name:v.homeTeam||'—'},away:{name:v.awayTeam||'—'}},
      league:{id:v.leagueId,name:v.league||''}
    },
    hXGfinal:hPred, aXGfinal:aPred, tXG:_pmHasNum(v.tXG)?Number(v.tXG):null,
    xgDiff:_pmHasNum(v.xgDiff)?Number(v.xgDiff):null,
    expCor:_pmHasNum(v.pmExpCor)?Number(v.pmExpCor):null,
    cornerConf:_pmHasNum(v.preCornerConf)?Number(v.preCornerConf):null,
    hS:_pmHasNum(v.pmCardH)?{crd:Number(v.pmCardH)}:{},
    aS:_pmHasNum(v.pmCardA)?{crd:Number(v.pmCardA)}:{},
    actStats:v.pmActStats||null,
    outPick:v.outPick||'-', outcomeMarket:v.preOutcomeMarket||v.outPick||'-', outcomeProb:_pmHasNum(v.preOutcomeProb)?Number(v.preOutcomeProb):null,
    exact:v.exactScorePred||'?-?', exact2:v.exactScorePred2||null, exactConf:_pmHasNum(v.preExactConf)?Number(v.preExactConf):null,
    omegaPick:v.omegaPick||'ΧΩΡΙΣ ΣΥΣΤΑΣΗ', strength:Number(v.strength||0), isBomb:!!v.isBomb,
    prePHome:v.prePHome, prePDraw:v.prePDraw, prePAway:v.prePAway,
    preConfirmedSignals:Array.isArray(v.preConfirmedSignals)?v.preConfirmedSignals:[],
    isCup:!!v.isCup, homeCategory:v.homeCategory||null, awayCategory:v.awayCategory||null
  };
}
function _pmDataset(currentFinished){
  const allVault=_pmVault();
  if(postMatchRange==='CURRENT'){
    return {
      matches:currentFinished,
      totalRecords:currentFinished.length,
      settledCount:currentFinished.length,
      fullStatsCount:currentFinished.filter(x=>!!x.actStats).length,
      pendingCount:0,
      legacyCount:0,
      anyVaultCount:allVault.length,
      label:'Current Scan'
    };
  }
  const records=_pmRecordsForRange(postMatchRange);
  const matches=records.map(_pmVaultToMatch).filter(Boolean);
  return {
    matches,
    records,
    totalRecords:records.length,
    settledCount:matches.length,
    fullStatsCount:matches.filter(x=>!!x.actStats).length,
    pendingCount:records.filter(r=>!_pmParseScore(r.pmActualScore) || !r.pmActStats).length,
    legacyCount:records.filter(r=>Number(r.pmSchema||0)<2).length,
    anyVaultCount:allVault.length,
    label:_pmRangeLabel(postMatchRange)
  };
}
window.setPostMatchRange=function(v){
  const value=POSTMATCH_RANGES.has(String(v))?String(v):'CURRENT';
  postMatchRange=value;
  try { localStorage.setItem(LS_POSTMATCH_RANGE,value); } catch {}
  renderSummaryTable();
};
function _pmBuildActStats(statsResponse, goals){
  if(!Array.isArray(statsResponse) || statsResponse.length<2) return null;
  const hs=statsResponse[0]?.statistics||[], as=statsResponse[1]?.statistics||[];
  const hasStat=(arr,type)=>(arr||[]).some(z=>z?.type===type && z?.value!==null && z?.value!==undefined && z?.value!=='');
  const out={
    hPoss:statVal(hs,'Ball Possession'), aPoss:statVal(as,'Ball Possession'),
    hCor:statVal(hs,'Corner Kicks'), aCor:statVal(as,'Corner Kicks'),
    hCrd:statVal(hs,'Yellow Cards')+statVal(hs,'Red Cards'),
    ...
    hSoT:statVal(hs,'Shots on Goal'), aSoT:statVal(as,'Shots on Goal'),
    hFoul:statVal(hs,'Fouls'), aFoul:statVal(as,'Fouls'),
    _hasXG:hasStat(hs,'expected_goals')&&hasStat(as,'expected_goals'),
    _hasCorners:hasStat(hs,'Corner Kicks')&&hasStat(as,'Corner Kicks'),
    _hasCards:(hasStat(hs,'Yellow Cards')||hasStat(hs,'Red Cards'))&&(hasStat(as,'Yellow Cards')||hasStat(as,'Red Cards'))
  };
  out.totCor=(out.hCor||0)+(out.aCor||0);
  out.totCrd=(out.hCrd||0)+(out.aCrd||0);
  out.totOff=(out.hOff||0)+(out.aOff||0);
  out.totGoals=(goals?.home||0)+(goals?.away||0);
  out.btts=(goals?.home||0)>0&&(goals?.away||0)>0;
  return out;
}
window.syncPostMatchHistory=async function(){
  if(postMatchSyncing || isRunning || postMatchRange==='CURRENT') return;
  const records=_pmRecordsForRange(postMatchRange);
  const missing=records.filter(r=>!_pmParseScore(r.pmActualScore) || !r.pmActStats);
  if(!missing.length){ showOk('✅ Το επιλεγμένο Post-Match ιστορικό είναι ήδη συγχρονισμένο.'); return; }

  // Quota guard: έως 60 fixtures ανά πάτημα. Επαναλαμβάνεται με δεύτερο πάτημα αν χρειάζεται.
  const MAX_SYNC=60;
  const work=missing.slice(0,MAX_SYNC);
  postMatchSyncing=true;
  isRunning=true;
  setBtnsDisabled(true);
  renderSummaryTable();
  setLoader(true,`Post-Match sync 0/${work.length}…`);
  let done=0, settled=0, full=0;
  try{
    const store=_pmVault();
    ...
    for(let i=0;i<work.length;i+=BATCH){
      ...
      await Promise.all(batch.map(async rec=>{
        try{
          let score=_pmParseScore(rec.pmActualScore);
          let fix=null;
          if(!score){
            const fr=await apiReq(`fixtures?id=${rec.fixtureId}`);
            fix=fr?.response?.[0]||null;
            if(!fix || !isFinished(fix?.fixture?.status?.short)) return;
            score={h:Number(fix.goals?.home||0),a:Number(fix.goals?.away||0)};
          }
          settled++;
          let act=rec.pmActStats||null;
          if(!act){
            const sr=await getFixStats(rec.fixtureId);
            act=_pmBuildActStats(sr,{home:score.h,away:score.a});
            if(act) full++;
          } else full++;
          const prev=map.get(String(rec.fixtureId))||rec;
          map.set(String(rec.fixtureId),{
            ...prev,
            pmActualScore:`${score.h}-${score.a}`,
            pmActualHome:score.h, pmActualAway:score.a,
            pmStatus:'FT', pmActStats:act||prev.pmActStats||null,
            pmSettledAt:new Date().toISOString()
          });
        }catch(e){ console.warn('[APEX] Post-Match sync fixture',rec.fixtureId,e.message); }
        finally{
          done++;
          setProgress(Math.round(done/work.length*100),`Post-Match sync ${done}/${work.length}`);
        }
      }));
    }
    localStorage.setItem(LS_PREDS,JSON.stringify(Array.from(map.values())));
    const left=Math.max(0,missing.length-work.length);
    showOk(`✅ Post-Match: ${settled} ολοκληρωμένοι · ${full} με πλήρη stats${left?` · ${left} απομένουν για επόμενο sync`:''}`);
  }catch(e){ showErr('Post-Match sync: '+e.message); }
  finally{
    postMatchSyncing=false;
    isRunning=false;
...
function renderSummaryTable() {
  ...
  const currentFinished = sd.filter(d => isFinished(d.m?.fixture?.status?.short));
  const postMeta = _pmDataset(currentFinished);
  const finishedMatches = postMeta.matches;
  if(!sd.length && !postMeta.anyVaultCount) { sec.innerHTML=''; return; }
  ...
  if (activeMatches.length > 0) {
    ...
    finalHtml += `<div class="quant-panel" style="padding:0;overflow:hidden;">
      <div style="padding:15px 20px;border-bottom:1px solid var(--border-light);display:flex;align-items:center;justify-content:space-between;">
        <span style="font-size:0.95rem;font-weight:800;color:var(--accent-blue);text-transform:uppercase;letter-spacing:1px;">📊 Match Dashboard (Active) — ${activeMatches.length} αγώνες</span><span style="font-size:.64rem;color:var(--text-muted);font-family:var(--font-mono);">🕒 όλες οι ώρες: Ελλάδα (Europe/Athens)</span>
      </div>${rows}</div>`;
  }

  // 2. FINISHED MATCHES / POST-MATCH HISTORY
  if (finishedMatches.length > 0 || postMeta.anyVaultCount > 0) {
    let fRows = '';
    finishedMatches.forEach(x => {
      const ah = Number(x.m?.goals?.home??0), aa = Number(x.m?.goals?.away??0);
      const aTot = ah+aa, aOut = ah>aa?'1':ah<aa?'2':'X', aBtts = ah>0&&aa>0;
      const hasAct=!!x.actStats;
      const hasXGPred=_pmHasNum(x.hXGfinal)&&_pmHasNum(x.aXGfinal);
      const hasCorPred=_pmHasNum(x.expCor)&&Number(x.expCor)>0;
      const hasCardPred=_pmHasNum(x.hS?.crd)&&_pmHasNum(x.aS?.crd);
      const hasCorAct=hasAct&&x.actStats?._hasCorners!==false&&_pmHasNum(x.actStats?.hCor)&&_pmHasNum(x.actStats?.aCor);
      const hasCrdAct=hasAct&&x.actStats?._hasCards!==false&&_pmHasNum(x.actStats?.hCrd)&&_pmHasNum(x.actStats?.aCrd);
      const hasXGAct=hasAct&&x.actStats?._hasXG!==false&&_pmHasNum(x.actStats?.hXg)&&_pmHasNum(x.actStats?.aXg);

      const hXGAct=hasXGAct?Number(x.actStats.hXg).toFixed(2):'—';
      const aXGAct=hasXGAct?Number(x.actStats.aXg).toFixed(2):'—';
      const hPoss=hasAct&&_pmHasNum(x.actStats?.hPoss)?x.actStats.hPoss:'—';
      const aPoss=hasAct&&_pmHasNum(x.actStats?.aPoss)?x.actStats.aPoss:'—';
      const hCorAct=hasCorAct?Number(x.actStats.hCor):null, aCorAct=hasCorAct?Number(x.actStats.aCor):null;
      const hCrdAct=hasCrdAct?Number(x.actStats.hCrd):null, aCrdAct=hasCrdAct?Number(x.actStats.aCrd):null;

      const hXGPred=hasXGPred?Number(x.hXGfinal).toFixed(2):'—';
      const aXGPred=hasXGPred?Number(x.aXGfinal).toFixed(2):'—';
      const tXGPred=hasXGPred?(Number(x.hXGfinal)+Number(x.aXGfinal)).toFixed(2):'—';
      const hCorPred=hasCorPred?Number(x.hProjCor??Number(x.expCor)/2).toFixed(1):'—';
      const aCorPred=hasCorPred?Number(x.aProjCor??Number(x.expCor)/2).toFixed(1):'—';
      const expCorPred=hasCorPred?Number(x.expCor).toFixed(1):'—';
      const hCrdPred=hasCardPred?Number(x.hS.crd).toFixed(1):'—';
      const aCrdPred=hasCardPred?Number(x.aS.crd).toFixed(1):'—';
      const totCrdPred=hasCardPred?(Number(x.hS.crd)+Number(x.aS.crd)).toFixed(1):'—';
      const totCrdAct=hasCrdAct?hCrdAct+aCrdAct:null;

      const xgDev=(hasXGAct&&hasXGPred)?Math.abs((Number(hXGAct)+Number(aXGAct))-Number(tXGPred)):null;
      const xgCol=xgDev===null?'var(--text-muted)':xgDev<0.5?'var(--accent-green)':xgDev<1.0?'var(--accent-gold)':'var(--accent-red)';
      const corDev=(hasCorAct&&hasCorPred)?Math.abs((hCorAct+aCorAct)-Number(expCorPred)):null;
      const corCol=corDev===null?'var(--text-muted)':corDev<2?'var(--accent-green)':corDev<4?'var(--accent-gold)':'var(--accent-red)';
      const crdDev=(hasCrdAct&&hasCardPred)?Math.abs(totCrdAct-Number(totCrdPred)):null;
      const crdCol=crdDev===null?'var(--text-muted)':crdDev<1.5?'var(--accent-green)':crdDev<3?'var(--accent-gold)':'var(--accent-red)';
      ...
      if(pick && !pick.includes('ΧΩΡΙΣ') && !pick.includes('NO BET')) {
        ...
        else if(pick.includes('ΚΟΡΝΕΡ')&&hasCorAct) hit=(hCorAct+aCorAct)>8.5;
        else if(pick.includes('ΚΑΡΤΕΣ')&&hasCrdAct) hit=(hCrdAct+aCrdAct)>5.5;
        else if(pick.includes('AH')){
          if(pick.includes('ΑΣΟΣ')) hit=(ah-aa)>=2;
          if(pick.includes('ΔΙΠΛΟ')) hit=(aa-ah)>=2;
        }
        if(hit!==null) hitHtml=hit
          ? `<span style="background:rgba(74,222,128,0.15);color:var(--accent-green);padding:3px 8px;border-radius:5px;font-weight:800;font-size:0.72rem;">✅ ΕΠΙΒΕΒΑΙΩΘΗΚΕ</span>`
          : `<span style="background:rgba(251,113,133,0.15);color:var(--accent-red);padding:3px 8px;border-radius:5px;font-weight:800;font-size:0.72rem;">❌ ΔΕΝ ΕΠΙΒΕΒΑΙΩΘΗΚΕ</span>`;
      }

      const pvA=(pred,actual,col='var(--text-main)')=>`<div style="font-family:var(--font-mono);line-height:1.4;"><div style="font-size:0.72rem;color:var(--text-muted);">📐 ${pred}</div><div style="font-size:0.92rem;font-weight:800;color:${col};">✔ ${actual}</div></div>`;
      const dateTxt=formatGreeceKickoff(x,{withDate:true});
      const rowAttrs=x._fromVault?`onclick="window.openMatchAnalysisDrawer('${x.fixId}')" style="cursor:pointer;" onmouseover="this.style.background='rgba(77,184,255,0.04)'" onmouseout="this.style.background=''"`:`onclick="window.openMatchAnalysisDrawer('${x.fixId}')" style="cursor:pointer;" onmouseover="this.style.background='rgba(77,184,255,0.04)'" onmouseout="this.style.background=''"`;
      const settlements=confirmedMarketsSettlementHTML(x,{compact:true});
      const strength=Number(x.strength||0);

      fRows += `<tr id="row-${x.fixId}" ${rowAttrs}>
        <td class="left-align" style="font-weight:700;font-size:0.95rem;min-width:190px;">
          <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;"><span>${esc(x.ht)}</span><span style="color:var(--text-dim);">vs</span><span>${esc(x.at)}</span></div>
          ${competitionContextHTML(x,{compact:true})}
          ${x._fromVault?'<div style="font-size:.6rem;color:var(--text-muted);margin-top:2px;">Vault history</div>':''}
        </td>
        <td style="text-align:center;"><div class="postmatch-final-label">ΤΕΛΙΚΟ</div><div class="postmatch-final-score">${ah}–${aa}</div><div style="font-size:0.6rem;color:var(--text-muted);">${aTot} γκολ</div></td>
        <td>${pvA(`${hXGPred}–${aXGPred} (${tXGPred})`,hasXGAct?`${hXGAct}–${aXGAct}`:'—',xgCol)}</td>
        <td style="text-align:center;font-family:var(--font-mono);"><div style="font-size:0.72rem;color:var(--text-muted);">—</div><div style="font-size:0.9rem;font-weight:700;">${hPoss==='—'?'—':`${hPoss}%–${aPoss}%`}</div></td>
        <td>${pvA(hasCorPred?`${hCorPred}–${aCorPred} (${expCorPred})`:'—',hasCorAct?`${hCorAct}–${aCorAct} (${hCorAct+aCorAct})`:'—',corCol)}</td>
        <td>${pvA(hasCardPred?`${hCrdPred}–${aCrdPred} (${totCrdPred})`:'—',hasCrdAct?`${hCrdAct}–${aCrdAct} (${totCrdAct})`:'—',crdCol)}</td>
        <td style="font-size:0.78rem;font-weight:700;color:${strength>=70?'var(--accent-green)':'var(--text-muted)'};max-width:140px;">${esc(pick.split(' ').slice(0,4).join(' ')||'—')}${strength>=70?`<div style="font-size:0.6rem;color:var(--text-muted);">${strength.toFixed(0)}% conf</div>`:''}</td>
        <td style="min-width:240px;">${settlements}</td>
        <td>${hitHtml}</td>
      </tr>`;
    });

    if(!fRows) fRows=`<tr><td colspan="9" style="padding:24px;text-align:center;color:var(--text-muted);">Δεν υπάρχουν ακόμη συγχρονισμένοι ολοκληρωμένοι αγώνες σε αυτό το εύρος.</td></tr>`;

    // ── Model Accuracy Analysis — per-metric valid samples, never fill missing values with zero
    const metrics={
      xGH:{label:'xG HOME',pred:[],actual:[]}, xGA:{label:'xG AWAY',pred:[],actual:[]},
      tXG:{label:'Total xG',pred:[],actual:[]}, corners:{label:'Κόρνερ (Σύν.)',pred:[],actual:[]},
      cards:{label:'Κάρτες (Σύν.)',pred:[],actual:[]}, goals:{label:'Γκολ (Σύν.)',pred:[],actual:[]}
    };
    finishedMatches.forEach(x=>{
      const ah=Number(x.m?.goals?.home??0), aa=Number(x.m?.goals?.away??0);
      const hasPred=_pmHasNum(x.hXGfinal)&&_pmHasNum(x.aXGfinal);
      const act=x.actStats||null;
      if(hasPred && act && act._hasXG!==false && _pmHasNum(act.hXg)&&_pmHasNum(act.aXg)){
        const hp=Number(x.hXGfinal), ap=Number(x.aXGfinal), ha=Number(act.hXg), aaX=Number(act.aXg);
        metrics.xGH.pred.push(hp); metrics.xGH.actual.push(ha);
        metrics.xGA.pred.push(ap); metrics.xGA.actual.push(aaX);
        metrics.tXG.pred.push(hp+ap); metrics.tXG.actual.push(ha+aaX);
      }
      if(hasPred){ metrics.goals.pred.push(Number(x.hXGfinal)+Number(x.aXGfinal)); metrics.goals.actual.push(ah+aa); }
      if(act && act._hasCorners!==false && _pmHasNum(x.expCor)&&Number(x.expCor)>0 && _pmHasNum(act.hCor)&&_pmHasNum(act.aCor)){
        metrics.corners.pred.push(Number(x.expCor)); metrics.corners.actual.push(Number(act.hCor)+Number(act.aCor));
      }
      if(act && act._hasCards!==false && _pmHasNum(x.hS?.crd)&&_pmHasNum(x.aS?.crd)&&_pmHasNum(act.hCrd)&&_pmHasNum(act.aCrd)){
        metrics.cards.pred.push(Number(x.hS.crd)+Number(x.aS.crd)); metrics.cards.actual.push(Number(act.hCrd)+Number(act.aCrd));
      }
    });

    const calcMAE=(p,a)=>p.length?p.reduce((s,v,i)=>s+Math.abs(v-a[i]),0)/p.length:null;
    const calcBias=(p,a)=>p.length?p.reduce((s,v,i)=>s+(v-a[i]),0)/p.length:null;
    const calcCorr=(p,a)=>{
      const n=p.length; if(n<3) return null;
      const mp=p.reduce((x,y)=>x+y,0)/n, ma=a.reduce((x,y)=>x+y,0)/n;
      const cov=p.reduce((s,v,i)=>s+(v-mp)*(a[i]-ma),0)/n;
      const sp=Math.sqrt(p.reduce((s,v)=>s+(v-mp)**2,0)/n), sa=Math.sqrt(a.reduce((s,v)=>s+(v-ma)**2,0)/n);
      return sp*sa>0?cov/(sp*sa):null;
    };
    const metricEntries=Object.values(metrics);
    const maxN=Math.max(0,...metricEntries.map(m=>m.pred.length));
    let accuracyHtml='';
    if(maxN>=2){
      const mCards=metricEntries.map(m=>{
        const n=m.pred.length, mae=calcMAE(m.pred,m.actual), bias=calcBias(m.pred,m.actual), corr=calcCorr(m.pred,m.actual);
        const corrText=corr===null?'N/A':`${(corr*100).toFixed(0)}%`;
        const corrCol=corr===null?'var(--text-muted)':corr>=0.7?'var(--accent-green)':corr>=0.4?'var(--accent-gold)':'var(--accent-red)';
        const biasCol=bias===null?'var(--text-muted)':Math.abs(bias)<0.3?'var(--accent-green)':Math.abs(bias)<0.7?'var(--accent-gold)':'var(--accent-red)';
        const biasStr=bias===null?'N/A':`${bias>0?'+':''}${bias.toFixed(2)} ${bias>0?'↑':bias<0?'↓':'↔'}`;
        const barW=corr===null?0:Math.min(Math.abs(corr)*100,100).toFixed(0);
        const maxV=n?Math.max(...m.pred,...m.actual,1):1;
        const dots=m.pred.map((p,i)=>{const xx=(p/maxV*60).toFixed(1), yy=(60-m.actual[i]/maxV*60).toFixed(1), dev=Math.abs(p-m.actual[i]);const dc=dev<0.5?'var(--accent-green)':dev<1.2?'var(--accent-gold)':'var(--accent-red)';return `<circle cx="${xx}" cy="${yy}" r="3.5" fill="${dc}" fill-opacity="0.8"/>`;}).join('');
        return `<div style="background:var(--bg-base);border:1px solid var(--border-light);border-radius:8px;padding:12px 14px;"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;gap:8px;"><span style="font-size:0.72rem;font-weight:800;color:var(--text-sub);font-family:var(--font-cond);text-transform:uppercase;letter-spacing:0.08em;">${m.label}</span><span style="font-size:0.6rem;color:var(--text-muted);font-family:var(--font-mono);">n=${n}</span><svg width="66" height="66" style="flex-shrink:0;border:1px solid var(--border-light);border-radius:5px;background:var(--bg-surface);" viewBox="-3 -3 66 66"><line x1="0" y1="60" x2="60" y2="0" stroke="rgba(255,255,255,0.15)" stroke-width="0.8" stroke-dasharray="3,2"/>${dots}</svg></div><div style="margin-bottom:8px;"><div style="display:flex;justify-content:space-between;font-size:0.6rem;color:var(--text-muted);margin-bottom:3px;"><span>Pearson r</span><span style="color:${corrCol};font-weight:700;">${corrText}</span></div><div style="height:4px;background:var(--border-light);border-radius:2px;"><div style="height:4px;width:${barW}%;background:${corrCol};border-radius:2px;"></div></div></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:0.68rem;"><div style="background:var(--bg-surface);border-radius:4px;padding:4px 6px;"><div style="color:var(--text-dim);font-size:0.58rem;">MAE</div><div style="font-family:var(--font-mono);font-weight:700;">${mae===null?'N/A':mae.toFixed(2)}</div></div><div style="background:var(--bg-surface);border-radius:4px;padding:4px 6px;"><div style="color:var(--text-dim);font-size:0.58rem;">Bias</div><div style="font-family:var(--font-mono);font-weight:700;color:${biasCol};">${biasStr}</div></div></div></div>`;
      }).join('');
      const ranked=metricEntries.map(m=>({label:m.label,n:m.pred.length,corr:calcCorr(m.pred,m.actual)})).filter(r=>r.corr!==null).sort((a,b)=>b.corr-a.corr);
      const rankHtml=ranked.length?ranked.map((r,i)=>{const col=r.corr>=0.7?'var(--accent-green)':r.corr>=0.4?'var(--accent-gold)':'var(--accent-red)';const medal=['🥇','🥈','🥉','4️⃣','5️⃣','6️⃣'][i]||`${i+1}.`;return `<div style="display:flex;align-items:center;gap:8px;padding:5px 0;border-bottom:1px solid rgba(255,255,255,0.04);"><span>${medal}</span><span style="font-size:0.72rem;color:var(--text-sub);flex:1;">${r.label} <span style="color:var(--text-dim);">(n=${r.n})</span></span><div style="width:80px;height:5px;background:var(--border-light);border-radius:2px;"><div style="height:5px;width:${Math.min(Math.abs(r.corr)*100,100).toFixed(0)}%;background:${col};border-radius:2px;"></div></div><span style="font-family:var(--font-mono);font-size:0.72rem;font-weight:700;color:${col};min-width:36px;text-align:right;">${(r.corr*100).toFixed(0)}%</span></div>`;}).join(''):`<div style="color:var(--text-muted);font-size:0.72rem;">Χρειάζονται ≥3 έγκυρες παρατηρήσεις ανά metric για Pearson r.</div>`;
      const sampleText=maxN<10?'⚠️ Πολύ μικρό δείγμα — μόνο διερευνητική ένδειξη':maxN<20?'⚠️ Προσωρινό δείγμα':maxN<30?'◐ Ενδεικτικό δείγμα':'✅ Επαρκέστερο δείγμα';
      accuracyHtml=`<div class="quant-panel" style="margin-top:16px;border-color:rgba(168,85,247,0.3);"><div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:16px;"><div><div style="font-size:0.85rem;font-weight:800;color:var(--accent-purple);font-family:var(--font-cond);text-transform:uppercase;letter-spacing:1px;">🔬 Model Accuracy Analysis</div><div style="font-size:0.65rem;color:var(--text-muted);margin-top:2px;">${sampleText} · κάθε metric χρησιμοποιεί μόνο διαθέσιμα πραγματικά δεδομένα</div></div></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:14px;"><div><div style="font-size:0.65rem;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.1em;font-family:var(--font-cond);margin-bottom:8px;">Ακρίβεια ανά Μέγεθος</div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:8px;">${mCards}</div></div><div><div style="font-size:0.65rem;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.1em;font-family:var(--font-cond);margin-bottom:8px;">Κατάταξη Συσχέτισης Π→Α</div><div style="background:var(--bg-base);border:1px solid var(--border-light);border-radius:8px;padding:12px 14px;margin-bottom:12px;">${rankHtml}</div><div style="background:rgba(168,85,247,0.06);border:1px solid rgba(168,85,247,0.2);border-radius:8px;padding:10px 14px;font-size:0.72rem;color:var(--text-muted);line-height:1.6;"><b style="color:var(--accent-purple);">Σημαντικό:</b> το Pearson r είναι συσχέτιση, όχι ποσοστό επιτυχίας. Το Post-Match ιστορικό είναι read-only και δεν αλλάζει τα thresholds του μοντέλου.</div></div></div></div>`;
    }

    const opt=(v,l)=>`<option value="${v}" ${postMatchRange===v?'selected':''}>${l}</option>`;
    const syncDisabled=postMatchRange==='CURRENT'||postMatchSyncing||postMeta.pendingCount===0;
    const syncTxt=postMatchSyncing?'⏳ Sync…':postMeta.pendingCount>0?`↻ Sync (${Math.min(postMeta.pendingCount,60)})`:'✓ Συγχρονισμένο';
    const legacyNote=postMeta.legacyCount>0?`<div style="font-size:0.62rem;color:var(--accent-gold);margin-top:4px;">⚠ ${postMeta.legacyCount} παλιές εγγραφές Vault: όπου δεν είχε αποθηκευτεί πρόβλεψη κόρνερ/καρτών εμφανίζεται «—» — δεν γίνεται τεχνητή ανακατασκευή.</div>`:'';
    finalHtml += `<div class="quant-panel" style="padding:0;overflow:hidden;margin-top:24px;border-color:rgba(74,222,128,0.35);"><div style="background:rgba(74,222,128,0.07);padding:12px 18px;border-bottom:1px solid var(--border-light);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;"><div><span style="font-size:0.82rem;font-weight:800;color:var(--accent-green);text-transform:uppercase;letter-spacing:1px;font-family:var(--font-cond);">🏁 Post-Match Evolution — ${finishedMatches.length} αγώνες</span><div style="font-size:0.62rem;color:var(--text-muted);margin-top:3px;">Πηγή: ${postMeta.label}${postMatchRange==='CURRENT'?'':` · Vault ${postMeta.settledCount}/${postMeta.totalRecords} settled · ${postMeta.fullStatsCount} με πλήρη stats`}</div>${legacyNote}</div><div style="display:flex;align-items:center;gap:7px;flex-wrap:wrap;"><select class="quant-input" style="height:32px;padding:4px 28px 4px 9px;width:auto;min-width:135px;font-size:0.72rem;" onchange="window.setPostMatchRange(this.value)">${opt('CURRENT','Current Scan')}${opt('7','7 ημέρες')}${opt('30','30 ημέρες')}${opt('90','90 ημέρες')}${opt('ALL','All Vault')}</select><button class="btn btn-outline" style="font-size:0.7rem;padding:6px 10px;" onclick="window.syncPostMatchHistory()" ${syncDisabled?'disabled':''}>${syncTxt}</button><span style="font-size:0.62rem;color:var(--text-muted);font-family:var(--font-mono);">📐 Πρόβλεψη · ✔ Πραγματικό</span></div></div><div class="data-table-wrapper" style="border:none;margin:0;"><table class="summary-table"><thead><tr><th class="left-align">Αγώνας</th><th>Σκορ</th><th>xG (Π→Α)</th><th>Possession</th><th>Κόρνερ (Π→Α)</th><th>Κάρτες (Π→Α)</th><th>Signal</th><th>Επιβεβαιώσεις Αγορών</th><th>Result</th></tr></thead><tbody>${fRows}</tbody></table></div></div>${accuracyHtml}`;
  }
...
function saveToVault(data){
  try{
    ...
    data.forEach(d=>{
      if(!d.fixId || !d.m) return;
      const id=String(d.fixId), prev=map.get(id)||{};
      const status=d.m?.fixture?.status?.short||'';
      const finished=isFinished(status);
      const gh=d.m?.goals?.home, ga=d.m?.goals?.away;
      const hasScore=finished && _pmHasNum(gh) && _pmHasNum(ga);
      const next={
        ...prev,
        fixtureId:d.fixId,
        ...
        preExactConf:Number.isFinite(Number(d.exactConf))?Number(d.exactConf):(prev.preExactConf??null),
        preOutcomeMarket:d.matchPicture?.recommendedMarket||d.outcomeMarket||prev.preOutcomeMarket||null,
        preOutcomeProb:Number.isFinite(Number(d.matchPicture?.recommendedProb??d.outcomeProb))?Number(d.matchPicture?.recommendedProb??d.outcomeProb):(prev.preOutcomeProb??null),
        preCornerConf:Number.isFinite(Number(d.cornerConf))?Number(d.cornerConf):(prev.preCornerConf??null),
        preConfirmedSignals:collectConfirmedMarketSignals(d),
        isCup:!!d.isCup,
        homeCategory:d.homeCategory||prev.homeCategory||null,
        awayCategory:d.awayCategory||prev.awayCategory||null,
        ...
        pmSchema:2,
        pmHXG:_pmHasNum(d.hXGfinal)?Number(d.hXGfinal):(prev.pmHXG??null),
        pmAXG:_pmHasNum(d.aXGfinal)?Number(d.aXGfinal):(prev.pmAXG??null),
        pmExpCor:_pmHasNum(d.expCor)?Number(d.expCor):(prev.pmExpCor??null),
        pmCardH:_pmHasNum(d.hS?.crd)?Number(d.hS.crd):(prev.pmCardH??null),
        pmCardA:_pmHasNum(d.aS?.crd)?Number(d.aS.crd):(prev.pmCardA??null),
        pmStatus:status||prev.pmStatus||null,
        pmActStats:d.actStats ? {...d.actStats} : (prev.pmActStats||null),
      };
      if(hasScore){
        next.pmActualHome=Number(gh); next.pmActualAway=Number(ga);
        next.pmActualScore=`${Number(gh)}-${Number(ga)}`;
        next.pmSettledAt=prev.pmSettledAt||new Date().toISOString();
      }
      map.set(id,next);
    });
    localStorage.setItem(LS_PREDS,JSON.stringify(Array.from(map.values())));
...
function updateLeagueFilterOption() {
  // My Leagues is now a Live Tracker preset; the pre-match scanner uses its own date-aware checkboxes.
...
// ================================================================
//  BANKROLL / BET JOURNAL SUPPORT
//  Το Verified Value Engine υπολογίζει EV/Kelly απευθείας από no-vig market consensus.
//  Εδώ παραμένει μόνο το κοινό fractional-Kelly constant και το Bet Journal state.
// ================================================================
...
function renderBetJournal() {
  ...
  const rows = betJournalData.slice(0, 50).map(b => {
    ...
    const resultBadge = b.result === 'won'    ? `<span style="color:var(--accent-green);font-weight:800;">✅ ΕΠΙΒΕΒΑΙΩΘΗΚΕ</span>`
                      : b.result === 'lost'   ? `<span style="color:var(--accent-red);font-weight:800;">❌ ΔΕΝ ΕΠΙΒΕΒΑΙΩΘΗΚΕ</span>`
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  const today=todayISO();
  const ss=document.getElementById('scanStart'), st=document.getElementById('scanStartTime'), se=document.getElementById('scanEnd'), et=document.getElementById('scanEndTime');
  const gp=_greeceDateTimeParts();
  if(ss)ss.value=today; if(st)st.value=`${gp.hour}:${gp.minute}`; if(se)se.value=today; if(et&&!et.value)et.value='23:59';
  ['scanStart','scanStartTime','scanEnd','scanEndTime'].forEach(id=>document.getElementById(id)?.addEventListener('change',scheduleScanLeagueRefresh));
