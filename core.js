...
async function _executeRequest(path,resolve) {
  ...
  try {
    ...
    if(typeof currentCredits==='number'){
      currentCredits--; 
      if(typeof updateCredits === 'function') updateCredits(currentCredits);
    }
...
function computeOverConfidence(hXG, aXG, pp, h2h, leagueId, hFormRating, aFormRating) {
  const tXG = hXG + aXG;
  const leagueAvg = LEAGUE_AVG_GOALS[leagueId] || 2.60;
  ...
  if(TIGHT_LEAGUES.has(leagueId)) score -= 4;
  if(GOLD_LEAGUES.has(leagueId))  score += 3;
...
function computeCornerConfidence(hCorAvg, aCorAvg, hXG, aXG, h2h) {
  ...
  if(combinedCor < engineConfig.minCorners) {
    score -= (engineConfig.minCorners - combinedCor) * 8;
  }
  return clamp(score, 0, 99);
}

function computePick(hXG,aXG,tXG,bttsScore,cor,totCards,lp,hS,aS,h2h,hInj=0,aInj=0,leagueId=0) {
  ...
  if(overConf35>=62 && safe_tXG>=lp.minXGO35 && safe_bttsScore>=1.20) {
  ...
  } else if(pp.pU25>=0.56 && safe_tXG<=lp.maxU25 && safe_bttsScore<=engineConfig.tBTTS_U25) {
  ...
  } else if(cornerConf>=65 && combinedCor>=engineConfig.minCorners) {
    ...
    omegaPick="🟨 OVER 5.5 ΚΑΡΤΕΣ"; pickScore=clamp((totCards - 5.0) * 20, 0, 85); reason=`Avg Cards: ${totCards.toFixed(1)} | Tension: ${Math.abs(xgDiff).toFixed(2)} xG`;
  }
